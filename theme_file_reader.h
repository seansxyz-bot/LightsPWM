...
public:
  ...
  static constexpr uint8_t MAX_THEME_NAME_LEN = 31;

  ThemeFileReader();

  void clear();

  bool commitTheme(uint8_t themeId, const char* name, const RGBColor* colors, uint8_t count);
  ...
  const char* themeName(uint8_t themeId) const;

  bool loadThemeFromDisk(uint8_t themeId);
  void loadAllFromDisk();

private:
  void setThemeName(uint8_t themeId, const char* name);

  RGBColor themes_[MAX_THEMES][MAX_COLORS_PER_THEME];
  uint8_t counts_[MAX_THEMES];
  bool valid_[MAX_THEMES];
  char names_[MAX_THEMES][MAX_THEME_NAME_LEN + 1];
};